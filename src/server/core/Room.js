...
class Room {
	...
	makeRoomList () {
		...
		for (let room of Object.keys(Rooms)) {
			packet += `rm id="${Rooms[room].ID}" priv="${roomObj.priv}" temp="${roomObj.temp}" game="${roomObj.game}" ucnt="${roomObj.ucnt}" scnt="${roomObj.scnt}" lmb="${roomObj.lmb}" maxu="${Rooms[room].maxUsers}" maxs="${roomObj.maxs}"><n><![CDATA[${Rooms[room].Name}]]></n></rm></rmList></body></msg>`
		}
		return packet
	}
}
