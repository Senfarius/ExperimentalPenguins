...
function handleJoin(roomData, id, res) {
	...
	database.getPlayerById(id).then(penguin => { return handleWrite(`&p=${penguin.room}${penguin.data.substr(1, 4)}${penguin.username}${Errors.OK}`, res) })
}
function handleChat(chatData, id, res) {
	...
	if (chatData.length > 2) {
		...
		database.getPlayerById(id).then(penguin => { return handleWrite(`&c=${penguin.room}${penguin.data}`, res) })
	}
}
